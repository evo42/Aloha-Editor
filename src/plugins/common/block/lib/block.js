 ...
 */
define(['aloha', 'jquery', 'block/blockmanager', 'aloha/observable', 'ui/scopes', 'util/class', 'PubSub'], function (
Aloha,
jQuery,
BlockManager,
Observable,
	...
	/** @lends block.block.AbstractBlock */ {
		...
		_constructor: function ($element, attributes) {
			...
			this._connectThisBlockToDomElement($element, function () {
				PubSub.pub('aloha.block.initialized', {
					block: that
				});
			});
		...
		_preventSelectionChangedEventHandler: function ($event) {
			if (('dblclick' !== $event.type) && !jQuery($event.target).is('.aloha-editable')) {
		...
		_disableUglyInternetExplorerDragHandles: function () {
			if (jQuery.browser.msie) {
				this.$element.get(0).onresizestart = function (e) {
					return false;
				};
				this.$element.get(0).oncontrolselect = function (e) {
					return false;
				};
				// We do NOT abort the "ondragstart" event as it is required for drag/drop.
				this.$element.get(0).onmovestart = function (e) {
					return false;
				};
		...
		_setupDragDropForInlineElements: function () {
			...
			var dropFn = function () {
				if (lastHoveredCharacter) {
					...
					$currentDraggable.removeClass('ui-draggable').css({
						'left': 0,
						'top': 0
					}); // Remove "draggable" options... somehow "Destroy" does not work
					that._fixScrollPositionBugsInIE();
				}
				...
				start: function () {
					...
					var droppableCfg = {
						...
						over: function (event, ui) {
							...
							1 === jQuery(this).contents().length && 1 === jQuery(this).children('br').length);
							$currentDraggable = ui.draggable;

							if (jQuery(this).is(':empty') || hasOnlyProppingBr || jQuery(this).html() === '&nbsp;') {
		...
		_dd_traverseDomTreeAndRemoveSpans: function (el) {
			...
			convertBack = function (el) {
				var currentlyTraversingExpandedText = false,
					currentText, lastNode;
				var child;
				for (var i = 0, l = el.childNodes.length; i < l; i++) {
					child = el.childNodes[i];
